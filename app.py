...
import re
from config.constants import TRANSACTION_CATEGORIES, CATEGORY_MAPPING
from services.google_sheets import initialize_sheets_service
from utils.logging_utils import configure_logging

log: Logger = configure_logging("financial_tracker")
...
def initialize_gemini_model() -> Any:
    """Initialize and cache Gemini AI configuration"""
    try:
        genai.configure(api_key=os.getenv('GEMINI_API_KEY')) # type: ignore
        ai_model: Any = genai.GenerativeModel('gemini-1.5-flash') # type: ignore
...
def get_category_structure() -> dict[str, dict[str, list[str]]]:
    """Cache the category structure to prevent reloading"""
    return CATEGORY_MAPPING

@st.cache_data
def get_transaction_categories() -> list[str]:
    """Cache transaction categories to prevent reloading"""
...
def extract_date_from_text(text: str) -> datetime:
    """
    Parse and extract date from natural language input.
...
def validate_sheet_connection() -> bool:
    """
    ...
    try:
        ...
        log.info("✅ Sheet connection validated successfully")
        return True
    except Exception as e:
        log.error(f"❌ Sheet connection validation failed: {str(e)}")
        return False

def setup_worksheet_structure() -> None:
    try:
        ...
        if not validate_sheet_connection():
            raise Exception("Failed to validate sheet connection")
            
        log.info("✨ Worksheets initialized and validated")
    except Exception as e:
        log.error(f"❌ Failed to initialize sheets: {str(e)}")
        raise

def save_transaction_to_sheet(date: str, amount: float, trans_type: str, 
                           category: str, subcategory: str, description: str) -> bool:
    """
    Save a new transaction to Google Sheet.
    ...
    Returns:
        bool: True if transaction saved successfully, False otherwise
    """
    try:
        log.info(f"Initiating transaction save: {date}, {amount}, {trans_type}, {category}, {subcategory}, {description}")
...
def fetch_transaction_data() -> pd.DataFrame:
    """
    Retrieve and process all transactions from Google Sheet.
    
    Returns:
        pd.DataFrame: Processed transactions data
    """
    try:
        log.debug("Retrieving transaction data from Google Sheets")
        ...
        log.error(f"❌ Failed to retrieve transaction data: {str(e)}")
        raise
...
def process_transaction_type(text: str, model: Any) -> dict[str, Any]:
    """
    Use Gemini AI to process and classify the transaction type.
    """
    try:
        ...
        log.debug("🤖 Sending processing prompt to Gemini")
        ...
        log.info(f"📋 Transaction processed as: {result.get('type', 'UNKNOWN')}")
        log.debug(f"Processing details: {result}")
        return result
    except Exception as e:
        log.error(f"❌ Failed to process transaction: {str(e)}")
        raise
...
def handle_received_pending_transaction(amount: float, description: str) -> tuple[bool, dict[str, Any] | None]:
    ...
    try:
        ...
        success = save_transaction_to_sheet(
...
def process_natural_language_input(text: str) -> dict[str, Any]:
    """
    Process natural language input to extract financial transaction details.
    """
    try:
        if not text or not text.strip():
            raise ValueError("Input text cannot be empty")
            
        log.info("🎯 Starting financial transaction processing")
        log.debug(f"Processing user input: {text}")
        
        # First, process the transaction type
        log.debug("Step 1: Processing transaction type")
        classification = process_transaction_type(text, model)
        ...
        log.info(f"Transaction processed as: {transaction_type}")
        
        # Handle each type differently
        if transaction_type == 'PENDING_RECEIVED':
            log.info("🔄 Processing received pending transaction")
        ...
        log.info("✅ Successfully processed financial transaction")
        log.debug(f"Final transaction info: {extracted_info}")
        return extracted_info
        
    except Exception as e:
        log.error(f"❌ Failed to process natural language input: {str(e)}", exc_info=True)
        raise

def display_analytics() -> None:
    """
    Generate analytics dashboard with transaction visualizations.
    Displays pie charts and trends for income and expenses.
    """
    try:
        log.info("Generating financial analytics dashboard")
        df = fetch_transaction_data()
        
        if df.empty:
            st.info("No transaction records yet. Add some transactions to view analytics!")
            return
        ...
        log.error(f"❌ Failed to generate analytics dashboard: {str(e)}")
        st.error("Failed to generate analytics dashboard. Please try again later.")
...
def verify_worksheet_setup() -> bool:
    try:
        result = service.spreadsheets().values().get(
            spreadsheetId=SHEET_ID,
            range='Expenses!A1:F1'
        ...
        if not values or values[0] != expected_headers:
            ...
            log.info("Headers re-initialized")
            
        return True
    except Exception as e:
        log.error(f"Failed to validate worksheet setup: {str(e)}")
        return False

def display_success_message(transaction_date: datetime | str, subcategory: str | None) -> None:
    """
    Show success message after transaction is saved.
    ...
    log.info("✅ Transaction saved and analytics refreshed")

def display_transaction_form():
    """Display and handle transaction form for user input"""
    ...
    if 'amount' in extracted_info and 'type' in extracted_info:
        ...
        with form_container:
            ...
            if st.session_state.form_submitted:
                try:
                    if extracted_info['type'] in ['To Receive', 'To Pay']:
                        success = save_pending_transaction_to_sheet(
...
def save_pending_transaction_to_sheet(date, amount, trans_type, category, description, due_date):
    try:
        # Validate sheets exist before adding transaction
        if not validate_sheets_setup():
            raise Exception("Failed to validate sheets setup")
            
        log.info(f"Initiating pending transaction save: {date}, {amount}, {trans_type}, {category}, {description}, {due_date}")
...
def validate_sheets_setup():
    """Validate both Expenses and Pending sheets exist with correct headers"""
    try:
        # Get all sheets
